...
const ShowroomSection = dynamic(() => import("@/components/showroom-section"), {
...
function ProductCardSkeleton() {
	return (
		<div className="flex-shrink-0 w-[180px] sm:w-[210px] md:w-[240px]">
			<div className="relative aspect-[3/4] overflow-hidden bg-gray-900 rounded-lg animate-pulse">
...
// Lazy video section — only plays when in viewport
function LazyVideoSection({ section, index }: { section: typeof videoSections[0]; index: number }) {
	...
	return (
		<section
			ref={sectionRef}
			className="sticky top-0 h-screen w-full overflow-hidden flex items-center justify-center border-t border-white/10 bg-black"
			style={{ zIndex: index + 1 }}
		>
			{/* Dark placeholder until video loads */}
			{!visible && <div className="absolute inset-0 bg-gray-950" />}
			...
			<div className="relative z-10 text-center max-w-4xl px-6">
				...
				<Button
					variant="link"
					asChild
					className="text-white border-b border-white pb-2 hover:text-gray-300 hover:border-gray-300 hover:no-underline tracking-widest text-sm rounded-none h-auto px-0"
				>
...
function FadeInSection({ children, className = "", style }: { children: React.ReactNode; className?: string; style?: React.CSSProperties }) {
	...
	return (
		<div className="bg-black">
			...
			<section className="relative h-[80vh] md:h-[85vh] lg:h-screen w-full overflow-hidden bg-black">
				<video
					className="absolute top-0 left-0 w-full h-full object-cover opacity-80 hidden md:block"
					preload="metadata"
					playsInline
					autoPlay
					muted
				...
				<video
					className="absolute top-0 left-0 w-full h-full object-cover opacity-80 md:hidden"
					preload="metadata"
					playsInline
					autoPlay
					muted
				...
				<div className="absolute inset-0 flex flex-col justify-center items-center text-center px-4">
					<p className="text-white tracking-[0.3em] text-sm md:text-2xl mb-6 animate-slide-up font-display">
						TIMELESS ELEGANCE
					</p>
					<h1
						className="text-7xl md:text-9xl lg:text-[12rem] text-white mb-8 animate-fade-in font-normal font-heading"
					...
					>
						LKB Jewellers brings you exceptional pieces that tell your story. Each creation is a testament to our
			...
			<section className="py-12 md:py-16 bg-black relative overflow-hidden">
									...
									>
										<div className="relative aspect-[3/4] overflow-hidden bg-black rounded-lg">
											...
											<div className="absolute top-3 left-3">
												<Badge className="bg-white/90 backdrop-blur-sm text-black text-xs font-bold tracking-wider uppercase hover:bg-white/90">
													New
												</Badge>
											</div>
										</div>
				...
				<div className="container mx-auto px-4 md:px-6 relative z-10">
					<div className="text-center mt-12">
						<Button
							asChild
							className="group bg-white text-black px-10 py-4 h-auto font-bold tracking-widest text-sm hover:shadow-2xl hover:shadow-white/50 rounded-none relative overflow-hidden hover:bg-white active:scale-95 transition-all duration-300"
						>
							<Link href="/shop">
			...
			{/* ===== STICKY VIDEO SECTIONS — play only when in viewport ===== */}
			<div className="relative w-full bg-black">
				{videoSections.map((section, index) => (
					<LazyVideoSection key={section.id} section={section} index={index} />
				))}
			</div>
				...
				<div className="container mx-auto relative z-10">
					<FadeInSection className="text-center mb-12">
						...
						<Button
							variant="link"
							asChild
							className="text-gray-500 hover:text-white text-xs tracking-widest uppercase pt-4 h-auto p-0 hover:no-underline group"
						>
							<Link href="/contact">
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 max-w-7xl mx-auto">
						{serviceCards.map((card, index) => (
							<FadeInSection key={card.id} style={{ transitionDelay: `${index * 100}ms` } as React.CSSProperties}>
								<Link
									href={
										card.id === "buy"
											? "/we-buy"
									...
									className="group relative h-[380px] md:h-[420px] lg:h-[450px] overflow-hidden cursor-pointer block"
								>
									<div className="relative h-full transition-all duration-500 group-hover:-translate-y-4 group-hover:scale-105" style={{ transformStyle: "preserve-3d" }}>
										...
										<div className="absolute inset-0 overflow-hidden">
											<Image
												...
												fill
												loading="lazy"
												className={"object-cover transition-all duration-700 group-hover:scale-110 group-hover:rotate-1 " + (card.hoverImage ? "group-hover:opacity-0" : "")}
											...
											{card.hoverImage && (
												<Image
													...
													fill
													loading="lazy"
													className="object-cover transition-all duration-700 group-hover:scale-110 group-hover:rotate-1 opacity-0 group-hover:opacity-100"
													sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw"
												/>
											)}
										</div>
										...
										<div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6 md:px-8 z-20">
											...
											<p className="text-gray-300 text-xs md:text-sm tracking-[0.15em] md:tracking-[0.2em] uppercase font-display">
												{card.description}
											</p>
										</div>
				...
				<div className="container mx-auto relative z-10 max-w-7xl">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
						{[
							...
							<FadeInSection key={cat.title} style={{ transitionDelay: `${index * 80}ms` } as React.CSSProperties}>
								<Link href={cat.href} className="group relative h-[300px] md:h-[320px] lg:h-[350px] overflow-hidden block rounded-lg">
									<div className="absolute inset-0 overflow-hidden">
										<Image
											src={cat.img}
											alt={cat.alt}
				...
				<div className="container mx-auto px-4 md:px-6 relative z-10">
					<div className="max-w-4xl mx-auto">
						<FadeInSection className="text-center mb-12">
							...
							<h2 className="text-5xl md:text-6xl lg:text-7xl text-white mb-6 leading-tight font-normal font-heading uppercase">
								JOIN OUR CIRCLE
							</h2>
							<p className="text-gray-400 text-lg md:text-xl max-w-2xl mx-auto leading-relaxed font-display">
								Subscribe to receive exclusive updates on new collections, private sales, and insider access to the world
								of LKB Jewellers.
							</p>
						</FadeInSection>
								...
								<form onSubmit={handleSubscribe} className="flex flex-col md:flex-row gap-3 md:gap-4">
									<div className="flex-1 relative group">
										<Input
											...
											disabled={loading}
											className="w-full px-4 md:px-6 py-3 md:py-4 h-auto bg-black/50 border-gray-700 rounded-lg text-white placeholder:text-gray-500 text-base focus-visible:border-white focus-visible:ring-white/20"
											style={{ fontSize: "16px" }}
											required
										/>
									...
									<Button
										type="submit"
										disabled={loading}
										className="group relative px-6 md:px-8 py-3 md:py-4 h-auto bg-white text-black font-bold tracking-widest text-xs md:text-sm rounded-lg overflow-hidden hover:shadow-2xl hover:shadow-white/50 hover:scale-105 hover:bg-white active:scale-95 transition-all duration-300"
									>
						...
						<div className="grid grid-cols-3 gap-4 md:gap-6 lg:gap-8 mt-8 md:mt-12 px-2 md:px-4">
							{[
								...
								<div key={stat.title} className="text-center">
									...
									<h3 className="text-white font-semibold mb-1 md:mb-2 text-xs sm:text-sm md:text-base font-heading uppercase">
										{stat.title}
									</h3>
			...
			{/* ===== VISIT OUR SHOWROOM — lazy loaded ===== */}
			<ShowroomSection />
		</div>
	);
}
